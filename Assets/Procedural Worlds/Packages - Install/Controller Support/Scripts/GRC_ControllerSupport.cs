...
namespace Gaia
{
    ...
    public class GRCControllerSupport : GaiaRuntimeComponent
    {
        ...
        //Reason: Reference to the controller settings object which does not need to reset with playmode changes
#pragma warning disable UDR0001
        private static GRC_ControllerSettings m_grcControllerSettings;
#pragma warning restore UDR0001
        public bool m_overrideTargetLocation;
        ...
        {
            if (m_grcControllerSettings == null)
            {
                m_grcControllerSettings = GaiaUtils.GetAsset("Controller Settings.asset", typeof(GRC_ControllerSettings)) as GRC_ControllerSettings;
            }
            return m_grcControllerSettings; 
        }

        public static void SetControllerSettings(GRC_ControllerSettings gRC_ControllerSettings)
        { 
            m_grcControllerSettings = gRC_ControllerSettings;
        }
        ...
        public override void AddToScene()
        {
            ...
            if (m_selectedControllerType != ControllerType.Custom)
            {
                gaiaPlayerObj = GaiaUtils.GetPlayerObject(true, true);
            }
            ...
            if (m_enableDynamicResolution)
            {
                ...
                if (!dynRes.enabled)
                {
                    ...
                    if (dynRes.advancedUpscalerNames.Count == 0)
                    {
                        //STP should work without package install
                        dynRes.advancedUpscalerNames.Add(UnityEngine.Rendering.AdvancedUpscalers.STP.ToString());
        ...
        public override void RemoveFromScene()
        {
            ...
            {
                //Remove all tags from cameras within the player object - this is to avoid race conditions if immediately afterwards another player is created
                //and finds the older, undeleted camera still tagged as "MainCamera"
                foreach (Camera cam in gaiaPlayerObj.transform.GetComponentsInChildren<Camera>(true))
                {
                    cam.tag = "Untagged";
                }
