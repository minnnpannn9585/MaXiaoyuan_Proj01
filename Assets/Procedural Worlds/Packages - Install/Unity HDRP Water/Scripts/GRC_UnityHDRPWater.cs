...
namespace Gaia
{
    public class GRC_UnityHDRPWater : GaiaRuntimeComponent
    {
        ...
        public override void AddToScene()
        {
            ...
            foreach (var spawner in GaiaUtils.FindObjectsByTypeVersionFix<Spawner>())
            {
                spawner.m_showSeaLevelPlane = false;
            }

            foreach (var stamper in GaiaUtils.FindObjectsByTypeVersionFix<Stamper>())
            {
                stamper.m_showSeaLevelPlane = false;
            }

            foreach (var biomeController in GaiaUtils.FindObjectsByTypeVersionFix<BiomeController>())
            {
        ...
        private void ConfigureHDRPWater()
        {
            ...
            foreach (Volume v in GaiaUtils.FindObjectsByTypeVersionFix<Volume>())
            {
                if (!v.isGlobal)
                {
                    continue;
