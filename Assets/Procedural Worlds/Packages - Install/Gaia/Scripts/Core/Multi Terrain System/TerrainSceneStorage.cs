...
namespace Gaia
{
    ...
    public class TerrainSceneStorage : ScriptableObject
    {
        ...
        /// <summary>
        /// Returns one big bounding box that encapsulates all terrain tiles from the terrain scene storage.
        /// </summary>
        /// <returns></returns>
        public Bounds GetWorldBounds()
        { 
            Bounds result = new Bounds();
            foreach (TerrainScene terrainScene in m_terrainScenes)
            {
                result.Encapsulate(terrainScene.m_bounds);
            }
            return result;
        }
