...
#pragma warning restore 649
    ...
    public class Spawner : MonoBehaviour
    {
        ...
        //Reason: This bool is only used to flag that the spawning should stop across a coroutine. Even if a playmode change happens inbetween,
        //it would even be beneficial if it keeps that value
...
#pragma warning restore UDR0001
                    ...
                    if (m_settings.m_spawnerRules[i].m_isActive)
                    {
                        switch (m_settings.m_spawnerRules[i].m_resourceType)
                        {
                            ...
                            case GaiaConstants.SpawnerResourceType.TerrainTree:
                                ...
                                {
                                    //Ensure the prototype exists on the affected terrains with the correct settings
                                    //We use the GaiaResource method for this, as it contains the logic for NavMeshLOD etc.
                                    foreach (var entry in operation.affectedTerrainPixels.Where(x => x.Key.operationType == MultiTerrainOperationType.Tree && x.Value.simulationPositive == true).Select(x => x.Key.terrain).Distinct())
                                    {
                                        m_settings.m_resources.AddPrototypeToTerrain(SpawnerResourceType.TerrainTree, m_settings.m_spawnerRules[i].m_resourceIdx, entry);
                                    }
            ...
            for (xWU = xWUMin; xWU < xWUMax; xWU += m_locationIncrement)
            {
                for (zWU = zWUMin; zWU < zWUMax; zWU += m_locationIncrement)
                {
                    ...
                    if (CheckLocation(location, ref spawnInfo))
                    {
                        ...
                        {
                            List<HeightMap> txtMaps = spawnInfo.m_spawner.GetTextureMaps(GaiaUtils.GetInstanceIdVersionFix(spawnInfo.m_hitTerrain));
                            if (txtMaps != null)
                            {
        ...
        public void CreateSpawnCaches()
        {
            ...
            for (idx = 0; idx < m_settings.m_spawnerRules.Count; idx++)
            {
                if (m_settings.m_spawnerRules[idx].CacheTextures(this))
                {
                    foreach (Terrain t in Terrain.activeTerrains)
                    {
                        CacheTextureMapsFromTerrain(GaiaUtils.GetInstanceIdVersionFix(t));
                    }
            ...
            for (idx = 0; idx < m_settings.m_spawnerRules.Count; idx++)
            {
                if (m_settings.m_spawnerRules[idx].CacheDetails(this))
                {
                    foreach (Terrain t in Terrain.activeTerrains)
                    {
                        CacheDetailMapsFromTerrain(GaiaUtils.GetInstanceIdVersionFix(t));
                    }
                    m_cacheDetails = true;
                    break;
                }
            ...
            for (idx = 0; idx < m_settings.m_spawnerRules.Count; idx++)
            {
                ...
                {
                    CacheHeightMapFromTerrain(GaiaUtils.GetInstanceIdVersionFix(Terrain.activeTerrain));
        ...
        public void DeleteSpawnCaches(bool flushDirty = false)
        {
            ...
            if (m_cacheTextures)
            {
                if (flushDirty && m_textureMapsDirty && m_cancelSpawn != true)
                {
                    ...
                    foreach (Terrain t in Terrain.activeTerrains)
                    {
                        SaveTextureMapsToTerrain(GaiaUtils.GetInstanceIdVersionFix(t));
                    }
                }
            ...
            if (m_cacheDetails)
            {
                if (m_cancelSpawn != true)
                {
                    foreach (Terrain t in Terrain.activeTerrains)
                    {
                        SaveDetailMapsToTerrain(GaiaUtils.GetInstanceIdVersionFix(t));
                    }
                }
            ...
            if (m_cacheHeightMaps)
            {
                if (flushDirty && m_heightMapDirty && m_cancelSpawn != true)
                {
                    ...
                    SaveHeightMapToTerrain(GaiaUtils.GetInstanceIdVersionFix(Terrain.activeTerrain));
                }
        ...
        public bool CheckLocation(Vector3 locationWU, ref SpawnInfo spawnInfo)
        {
            ...
            if (Physics.Raycast(locationWU, Vector3.down, out m_checkHitInfo, Mathf.Infinity, m_spawnCollisionLayers))
            {
                ...
                if (terrain != null)
                {
                    ...
                    {
                        List<HeightMap> hms = m_textureMapCache[GaiaUtils.GetInstanceIdVersionFix(terrain)];
        ...
        public void CacheTextureMapsFromTerrain(int terrainID)
        {
            ...
            for (int terrIdx = 0; terrIdx < Terrain.activeTerrains.Length; terrIdx++)
            {
                ...
                if (GaiaUtils.GetInstanceIdVersionFix(terrain) == terrainID)
                {
        ...
        public void ActivateTreeStandIn(int m_spawnRuleIndexBeingDrawn)
        {
            ...
            foreach (Terrain t in Terrain.activeTerrains)
            {
                ...
                {
                    if (GaiaUtils.IsSameGameObject(resourceProtoTree.m_desktopPrefab, proto.prefab, false))
                    {
        ...
        public void DeactivateTreeStandIn(int m_spawnRuleIndexBeingDrawn)
        {
            ...
            foreach (Terrain t in Terrain.activeTerrains)
            {
                ...
                foreach (TreePrototype proto in t.terrainData.treePrototypes)
                {
                    if (GaiaUtils.IsSameGameObject(m_gaiaSettings.m_boxStandInPrefab, proto.prefab, false))
                    {
                        treePrototypeID = localTerrainIdx;
                        break;
                    }
