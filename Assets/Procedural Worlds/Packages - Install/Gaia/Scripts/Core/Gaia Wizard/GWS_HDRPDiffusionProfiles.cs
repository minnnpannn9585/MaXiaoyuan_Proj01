...
namespace Gaia
{
    public class GWS_HDRPDiffusionProfiles : GWSetting
    {
        ...
        public override bool PerformCheck()
        {
            ...
            VolumeProfile gaiaVP = GetDefaultVolumeProfile(upp);
            ...
            //Different ways to get the default volume profile in different unity versions.
#if UNITY_6000_0_OR_NEWER
            SerializedProperty settings = currentSettingsSO.FindProperty("m_Settings");

            RenderPipelineGraphicsSettingsContainer container = (RenderPipelineGraphicsSettingsContainer)settings.boxedValue;

            IRenderPipelineGraphicsSettings defaultVolumeProfileSettings = null;

            foreach (IRenderPipelineGraphicsSettings setting in container.settingsList)
            {
                if (setting.GetType() == typeof(HDRPDefaultVolumeProfileSettings))
                {
                    defaultVolumeProfileSettings = setting;
                }
            }
            HDRPDefaultVolumeProfileSettings hDRPDefaultVolumeSettings = (HDRPDefaultVolumeProfileSettings)defaultVolumeProfileSettings;

            VolumeProfile currentVP = hDRPDefaultVolumeSettings.volumeProfile;
        ...
        public HDRenderPipelineAsset GetHDRPAsset()
        {
            ...
            {
                return (HDRenderPipelineAsset)GaiaUtils.GetRenderPipelineAsset();
...
#if HDPipeline && UNITY_EDITOR
        public static VolumeProfile GetDefaultVolumeProfile(UnityPipelineProfile profile)
        {
            GaiaPackageVersion unityVersion = GaiaUtils.GetPackageVersion();
            UnityVersionPipelineAsset mapping = profile.m_highDefinitionPipelineProfiles.Find(x => x.m_unityVersion == unityVersion);

            string defaultVolumeProfileName = "";
            if (mapping != null)
            {
                defaultVolumeProfileName = mapping.m_defaultSettingsVolumeProfileName;
            }
            else
            {
                //No mapping? This is most likely a new, untested unity version. Try latest entry in this case since this is most likely to work.
                defaultVolumeProfileName = profile.m_highDefinitionPipelineProfiles.Last().m_defaultSettingsVolumeProfileName;
            }
            return AssetDatabase.LoadAssetAtPath<VolumeProfile>(GaiaUtils.GetAssetPath(defaultVolumeProfileName + GaiaConstants.gaiaFileFormatAsset));
