...
namespace Gaia
{
    public class GRC_Utilities : GaiaRuntimeComponent
    {
        ...
        {
            GameObject gaiaObj = GaiaUtils.GetRuntimeSceneObject();
            GameObject windZoneObj = null;
            WindZone globalWind = gaiaObj.GetComponentInChildren<WindZone>();
            if (globalWind == null)
            {
                windZoneObj = new GameObject("PW Wind Zone");
            ...
            {
                windZoneObj = globalWind.gameObject;
            }
            switch (m_windType)
            {
            ...
            //Add BOTD wind control, if present
            TryAttachComponent(windZoneObj, "WindControl");


        }

        void TryAttachComponent(GameObject targetObj, string className)
        {
            // Try to find the type by its full name.
            // If the class is in a namespace, you must include it (e.g. "MyNamespace.OptionalScript").
            Type targetType = Type.GetType(className);

            if (targetType == null)
            {
                // Try searching all loaded assemblies (Unity often requires this)
                foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
                {
                    targetType = asm.GetType(className);
                    if (targetType != null)
                        break;
                }
            }
            ...
            if (targetType == null)
            {
                //Debug.LogWarning($"[ConditionalAttach] Script '{className}' not found in project.");
                return;
            }

            // Check if it’s a Component before adding it
            if (!typeof(Component).IsAssignableFrom(targetType))
            {
                Debug.LogError($"Error when trying to add the BOTD Wind Control from Gaia: Type '{className}' exists but is not a Component.");
                return;
            }

            // Add it dynamically
            targetObj.AddComponent(targetType);
            //Debug.Log($"[ConditionalAttach] Successfully attached '{className}' to '{targetObj.name}'.");
        }
