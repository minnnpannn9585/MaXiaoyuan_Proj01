...
#endif
#if PW_STORM_PRESENT
using ProceduralWorlds.Storm.Geospatial;
#endif
...
namespace Gaia
{
    ...
    public class GaiaSessionManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Toggle to turn on / off session recording. Recording a session allows you to review and play back past
        /// operations like spawning and stamping etc. via the session manager, but also slows down the overall processing
        /// times in Gaia due to the creation of the session entries that collect the data.
        /// </summary>
        public bool m_recordSession = true;
        ...
        //Reason: Singleton / single static object reference that can be kept alive
...
#pragma warning restore UDR0001
        ...
        public static GaiaSessionManager GetSessionManager(bool pickupExistingTerrain = false, bool createSession = true)
        {
            ...
            if (sessionMgr == null && createSession)
            {
                ...
                sessionMgr.m_recordSession = false;
        ...
        public void DirtyMinMax()
        {
            m_session.m_terrainMinMaxCache.Clear();
        }
        ...
        public void AddDefaults(GaiaDefaults defaults)
        {
            ...
            //                m_session.m_defaults.m_content = PWCommon7.Utils.ReadAllBytes(m_session.m_defaults.m_fileName);
            //                SaveSession();
            //#endif
            //            }
        }
        ...
        public void ResetSession()
        {
            ...
            if (m_session.m_operations.Count > 1 && m_recordSession) 
            {
        ...
        private void RestoreOriginAndLoadRange()
        {
            ...
            //force the update on restore to make sure we get updated terrains when we restore
            TerrainLoaderManager.Instance.SetLoadingRange(m_loadRangeBackup, m_loadRangeBackupImpostor, true, true);
            TerrainLoaderManager.Instance.m_freeTerrainSlotsButtonPositions.Clear();
        }
        ...
        public static void Stamp(StamperSettings stamperSettings, bool executeNow = true, Stamper stamper = null, bool massStamp = false)
        {
            ...
            {
                //only temprorarily store the settings for the execution
                op.SetTemporary(stamperSettings);
            }
            //Play the operation
            if (executeNow)
            {
        ...
        public static void StampFromWorldMap(WorldMapStampSettings worldMapStampSettings, bool executeNow = true, Stamper stamper = null)
        {
            ...
            {
                //only temprorarily store the settings for the execution
                op.SetTemporary(worldMapStampSettings);
            }
        ...
        public static void ExportWorldMapToLocalMap(WorldCreationSettings worldCreationSettings, bool executeNow)
        {
            ...
            {
                //only temprorarily store the settings for the execution
                op.SetTemporary(worldCreationSettings);
            }
            ...
            if (executeNow)
            {
                ExecuteExportWorldMapToLocalMap(worldCreationSettings);
            }
        }
        ...
        public static void ExportMaskMap(ExportMaskMapOperationSettings exportMaskMapOperationSettings, bool executeNow, MaskMapExport maskMapExport)
        {
            ...
            {
                //only temprorarily store the settings for the execution
                op.SetTemporary(exportMaskMapOperationSettings);
            }
        ...
        public static void FlattenTerrain(List<string> terrainNames, bool executeNow)
        {
            ...
            {
                //only temprorarily store the settings for the execution
                op.SetTemporary(flattenSettings);
            }
        ...
        public static void RemoveNonBiomeResources(RemoveNonBiomeResourcesSettings removeNonBiomeResourcesSettings, bool executeNow, BiomeController biomeController = null)
        {
            ...
            {
                //only temprorarily store the settings for the execution
                op.SetTemporary(removeNonBiomeResourcesSettings);
            }
        ...
        /// <summary>
        /// Adds a single terrain tile to the world with the given world creation settings, etc.
        /// </summary>
        /// <param name="executeNow">If this operation should be executed right after being added to the session.</param>
        public static void CreateSingleTerrainTile(SingleTileCreationSettings singleTileCreationSettings, bool executeNow)
        {
            //Get the current session manager instance from the scene
            GaiaSessionManager sessionMgr = GaiaSessionManager.GetSessionManager();
            ...
            GaiaOperation op = new GaiaOperation();
            op.m_operationType = GaiaOperation.OperationType.CreateSingleTerrainTile;
            op.m_isActive = true;
            op.m_operationDateTime = DateTime.Now.ToString();
            op.m_description = "Create Single Terrain Tile";

            //Copy the  creation Settings so we have a separate object for serialization
            SingleTileCreationSettings newSingleTileCreationSettings = Instantiate(singleTileCreationSettings);
            ...
            if (newSingleTileCreationSettings.m_gaiaDefaults == null)
            {
                GaiaSettings gaiaSettings = GaiaUtils.GetGaiaSettings();
                newSingleTileCreationSettings.m_gaiaDefaults = gaiaSettings.m_currentDefaults;
            }

            //Defaults need to be re-instantiated as well as new object
            newSingleTileCreationSettings.m_gaiaDefaults = Instantiate(newSingleTileCreationSettings.m_gaiaDefaults);
            newSingleTileCreationSettings.m_gaiaDefaults.name = "Gaia Defaults";

            //Same for the terrain prototype arrays - those need to be independent (shallow) copies as they might be taken from another terrain that might change at a later point
            if (newSingleTileCreationSettings.m_terrainLayers != null)
            {
                newSingleTileCreationSettings.m_terrainLayers = (TerrainLayer[])newSingleTileCreationSettings.m_terrainLayers.Clone();
            }
            if (newSingleTileCreationSettings.m_terrainTrees != null)
            {
                newSingleTileCreationSettings.m_terrainTrees = (TreePrototype[])newSingleTileCreationSettings.m_terrainTrees.Clone();
            }
            if (newSingleTileCreationSettings.m_terrainDetails != null)
            {
                newSingleTileCreationSettings.m_terrainDetails = (DetailPrototype[])newSingleTileCreationSettings.m_terrainDetails.Clone();
            }

            if (sessionMgr.m_recordSession)
            {
                //create the saved version of the scriptable asset
                string scriptableObjectPath = sessionMgr.SaveOperationData(op, newSingleTileCreationSettings);


                //Add the defaults as sub-object to the session entry
#if UNITY_EDITOR
                AssetDatabase.AddObjectToAsset(newSingleTileCreationSettings.m_gaiaDefaults, scriptableObjectPath);
#endif
                sessionMgr.AddOperation(op);
            }
            else
            {
                //only temprorarily store the settings for the execution
                op.SetTemporary(newSingleTileCreationSettings);
            }

            if (executeNow)
            {
                ExecuteCreateSingleTerrainTile(op);
            }
        ...
        public static void ClearSpawns(ClearOperationSettings clearOperationSettings, SpawnerSettings spawnerSettings = null, bool executeNow = true, Spawner spawner = null)
        {
            ...
            {
                //only temprorarily store the settings for the execution
                op.SetTemporary(clearOperationSettings);
            }
        ...
        private static void StampUndoRedo(bool isRedo, List<string> terrainNames, bool executeNow = true, Stamper stamper = null)
        {
            ...
            {
                //only temprorarily store the settings for the execution
                op.SetTemporary(undoRedoSettings);
            }
        ...
        public static void ExternalSessionEntry(ScriptableObject scriptableObject, Action<ScriptableObject> actionToExecute, bool executeNow = true)
        {
            ...
            if (sessionMgr == null)
            {
                Debug.LogError("Gaia could not store an external session entry: No Session Manager found in the scene.");
                return;
            }
            ...
            {
                op.SetTemporary(scriptableObject);
            }
            ...
            if (executeNow)
            {
                sessionMgr.BackupOriginAndLoadRange();
                m_regenerateRun = false;
                actionToExecute(op.ExternalOperationScriptableObject);
            }
        }
        ...
        public static void Spawn(SpawnOperationSettings spawnOperationSettings, bool executeNow = true, List<Spawner> spawnerList = null)
        {
            ...
            {
                op.SetTemporary(spawnOperationSettings);
            }
        ...
        /// <summary>
        /// Create a terrain tile based on the given world creation settings settings
        /// </summary>
        ...
        private static GameObject CreateTile(int tx, int tz, WorldCreationSettings worldCreationSettings, ref Terrain[,] world, TerrainLayer[] terrainLayers, DetailPrototype[] terrainDetails, TreePrototype[] terrainTrees)
        {
            ...
            {
                terrainName = string.Format(GaiaConstants.worldMapTerrainPrefix + "_{0}_{1}-{2}", tx, tz, worldCreationSettings.m_dateTimeString);
            }
            else
            {
                terrainName = string.Format("Terrain_{0}_{1}-{2}", tx, tz, worldCreationSettings.m_dateTimeString);
            }

            Vector3 position = new Vector3(worldCreationSettings.m_tileSize * tx + m_offset.x, 0, worldCreationSettings.m_tileSize * tz + m_offset.y);

            Terrain newTerrain = CreateGaiaTerrain(terrainName, position, worldCreationSettings.m_tileSize, worldCreationSettings.m_tileHeight,  gaiaDefaults,terrainLayers,terrainDetails,terrainTrees);

            //Save the new tile
            world[tx, tz] = newTerrain;

            GaiaStopwatch.EndEvent("Terrain Tile Creation");
            return newTerrain.gameObject;
        }

        private static Terrain CreateGaiaTerrain(string terrainName, Vector3 position, int size, float height, GaiaDefaults gaiaDefaults, TerrainLayer[] terrainLayers, DetailPrototype[] terrainDetails, TreePrototype[] terrainTrees)
        {
            //Create the terrain
            Terrain terrain;
            ...
            {
                if (m_sessionManager.GaiaSettings != null && m_sessionManager.GaiaSettings.m_pipelineProfile != null)
                {
#if HDPipeline
                    if (m_sessionManager.GaiaSettings.m_pipelineProfile.m_highDefinitionTerrainMaterial != null)
                    {
                        terrain.materialTemplate = m_sessionManager.GaiaSettings.m_pipelineProfile.m_highDefinitionTerrainMaterial;
                    }
#elif UPPipeline
                    if (m_sessionManager.GaiaSettings.m_pipelineProfile.m_universalTerrainMaterial != null)
                    {
                        terrain.materialTemplate = m_sessionManager.GaiaSettings.m_pipelineProfile.m_universalTerrainMaterial;
                    }
#else
                    if (m_sessionManager.GaiaSettings.m_pipelineProfile.m_builtInTerrainMaterial != null)
                    {
                        terrain.materialTemplate = m_sessionManager.GaiaSettings.m_pipelineProfile.m_builtInTerrainMaterial;
                    }
#endif
                }
            }
        ...
        public static bool CreateOrUpdateWorld(WorldCreationSettings worldCreationSettings, bool executeNow = true, bool isUpdate = false)
        {
            ...
            {
                op.SetTemporary(newWorldCreationSettings);
            }
        ...
        public static bool ExecuteOperation(GaiaOperation op)
        {
            ...
            switch (op.m_operationType)
            {
                ...
                case GaiaOperation.OperationType.UpdateWorld:
                    ...
                    return true;
                case GaiaOperation.OperationType.CreateSingleTerrainTile:
                    return ExecuteCreateSingleTerrainTile(op);
                case GaiaOperation.OperationType.ClearWorld:
        ...
        private IEnumerator ExecuteCreateWorld(GaiaOperation op)
        {
            ...
            //store the position of the 0/0 terrain - even if it not exists, this can be used later to calculate positons of new terrains relative to the original 0/0 terrain

            //this will center terrain / world  at origin
            Vector2 offset = new Vector2(-worldCreationSettings.m_tileSize * worldCreationSettings.m_xTiles * 0.5f, -worldCreationSettings.m_tileSize * worldCreationSettings.m_zTiles * 0.5f);

            //apply additional offset, if any
            offset += worldCreationSettings.m_centerOffset;

            terrainSceneStorage.m_pos00X = offset.x;
            terrainSceneStorage.m_pos00Z = offset.y;
            ...
            if (!m_regenerateRun)
            {
                ...
                if (createNewTSStorage)
                {
                    ...
                    if (worldCreationSettings.m_createInScene)
                    {
                        ...
                        TerrainLoaderManager.Instance.SetLoadingRange(regularRange, regularRange * 3f, false);
                        BackupOriginAndLoadRange();
                ...
                //We should dirty the min max cache since terrain heights might have changed during the update, etc.
                GaiaSessionManager gaiaSessionManager = GetSessionManager();
                if (gaiaSessionManager != null)
                {
                    gaiaSessionManager.DirtyMinMax();
                }
        ...
        private IEnumerator ExecuteUpdateWorld(GaiaOperation op)
        {
            ...
            //Can we find a geo tag on the terrains? If yes, get the sea level from there
#if PW_STORM_PRESENT
            Terrain activeTerrain = Terrain.activeTerrain;
            if (activeTerrain != null)
            {
                GeoTerrainTag geoTerrainTag = activeTerrain.GetComponent<GeoTerrainTag>();
                if (geoTerrainTag != null)
                {
                    GaiaAPI.SetSeaLevel(geoTerrainTag.SeaLevelInWorldUnits);
                }
            }
#endif
            ...
            //Before we begin, we need to make sure we have terrain data objects for each terrain saved out in the project hierarchy

            //First Save the project - there might be newly created terrains where the terrain data file "do not exist" yet because the project has never been saved after the terrain creation.
            EditorApplication.ExecuteMenuItem("File/Save Project");

            //It could be that the user supplied terrains where the terrain data is instanced ("serialized with the scene")
            //rather than stored as separate objects in the project. In this case we need to save them out individually
            if (!GaiaUtils.HasDynamicLoadedTerrains())
            {
                if (Terrain.activeTerrain != null)
                {
                    string terrainDataPath = GaiaDirectories.GetTerrainDataScenePath();
                    string terrainLayerPath = GaiaDirectories.GetTerrainLayerPath();

                    for (int t = 0; t < Terrain.activeTerrains.Length; t++)
                    {
                        ProgressBar.Show(ProgressBarPriority.WorldCreation, "Saving existing terrain data", "Storing pre-existing data that has not been saved yet...", t, Terrain.activeTerrains.Length - 1, false, false);

                        Terrain terrain = Terrain.activeTerrains[t];
                        //same thing with the terrain layer files - those also need to be saved out as terrain layer files
                        for (int i = 0; i < terrain.terrainData.terrainLayers.Length; i++)
                        {
                            ...
                            if (string.IsNullOrEmpty(AssetDatabase.GetAssetPath(tl)))
                            {
                                AssetDatabase.CreateAsset(tl, $"{terrainLayerPath}/{terrain.name}_{i}.asset");
                            }
                        }

                        //Save out the terrain data file
                        if (string.IsNullOrEmpty(AssetDatabase.GetAssetPath(terrain.terrainData)))
                        {
                            AssetDatabase.CreateAsset(terrain.terrainData, $"{terrainDataPath}/{terrain.name}.asset");
                        }
                    }
                }
            }
                            ...
                            foreach (Terrain t in Terrain.activeTerrains)
                            {
                                if (newTerrainGrid[x, z].m_terrainDataGUID == AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(t.terrainData)))
                                {
                                    ...
                                    if (worldCreationSettings.m_createInScene)
                                    {
                                        ...
                                        terrainScene.m_xCoord = x;
                                        terrainScene.m_zCoord = z;
                                        terrainSceneStorage.m_terrainScenes.Add(terrainScene);
                                        if (x == 0 && z == 0)
                                        {
                                            terrainSceneStorage.m_pos00X = terrainScene.m_bounds.min.x;
                                            terrainSceneStorage.m_pos00Z = terrainScene.m_bounds.min.z;
                                        }
                                    }
#endif
                                    break;
            ...
            if (GaiaUtils.HasDynamicLoadedTerrains() && worldCreationSettings.m_createInScene)
            {
                ...
                TerrainLoaderManager.Instance.TerrainSceneStorage.m_useFloatingPointFix = worldCreationSettings.m_applyFloatingPointFix;
                ...
                AddTerrainScenesToBuildSettings(TerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainScenes);
            }
            else
            {
        ...
        public static void GetTerrainUpdateGrids(ref WorldCreationSettings worldCreationSettings, ref BoundsDouble oldBounds, ref TerrainGridReference[,] oldTerrainGrid, ref TerrainGridReference[,] newTerrainGrid)
        {
            ...
            if (GaiaUtils.HasDynamicLoadedTerrains())
            {
                ...
                TerrainLoaderManager.Instance.PopulateXZCoords();

                int minX = TerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainScenes.Min(x => x.m_xCoord);
                int minZ = TerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainScenes.Min(x => x.m_zCoord);
                ...
                foreach (TerrainScene terrainScene in TerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainScenes)
                {
                    ...
                    int xCoord = terrainScene.m_xCoord - minX;
                    int zCoord = terrainScene.m_zCoord - minZ;
        ...
        private static bool ExecuteCreateSingleTerrainTile(GaiaOperation op)
        {
            //Get the current session manager instance from the scene
            GaiaSessionManager sessionMgr = GaiaSessionManager.GetSessionManager();

            SingleTileCreationSettings singleTileCreationSettings = op.SingleTileCreationSettings;
            Terrain t = CreateGaiaTerrain(singleTileCreationSettings.m_terrainName, singleTileCreationSettings.m_position, singleTileCreationSettings.m_terrainSize, singleTileCreationSettings.m_terrainHeight, singleTileCreationSettings.m_gaiaDefaults, singleTileCreationSettings.m_terrainLayers, singleTileCreationSettings.m_terrainDetails, singleTileCreationSettings.m_terrainTrees);

            if (singleTileCreationSettings.m_createInTerrainScene)
            {
#if GAIA_2023_PRO
                //create a world creation settings object that is suitable for creation of the terrain scene for the given terrain
                //The Terrain SceneCreator only uses a couple of those settings, so most can be left empty for our purpose
                WorldCreationSettings wcSettings = ScriptableObject.CreateInstance<WorldCreationSettings>();
                wcSettings.m_applyFloatingPointFix = TerrainLoaderManager.Instance.TerrainSceneStorage.m_useFloatingPointFix;
                wcSettings.m_autoUnloadScenes = false;
                wcSettings.m_isWorldMap = false;

                TerrainScene terrainScene = TerrainSceneCreator.CreateTerrainScene(t.gameObject.scene, TerrainLoaderManager.Instance.TerrainSceneStorage, sessionMgr.m_session, t.gameObject, wcSettings);
                TerrainLoaderManager.Instance.TerrainSceneStorage.m_terrainScenes.Add(terrainScene);
                TerrainLoaderManager.Instance.SaveStorageData();
                AddTerrainScenesToBuildSettings(new List<TerrainScene>() { terrainScene });
#endif
            }
