...
using UnityEngine;
using System.Linq;
using static Gaia.PolyMask;

namespace Gaia
{
    //The odd numbering is for alphabetical ordering of the enum values without destroying references to saved settings files
    public enum ImageMaskOperation { CollisionMask = 5, DistanceMask = 1, GlobalSpawnerMaskStack=13, GrowAndShrink=12, HeightMask = 2, ImageMask = 0, NoiseMask = 4, PolyMask=16, SlopeMask = 3, Smooth = 8, StrengthTransform = 6, TerrainTexture = 9, WorldBiomeMask = 11, PROConcaveConvex = 10, PROHydraulicErosion = 7, PROCurvature = 15 }
    ...
    public enum ImageMaskOperationReduced { CollisionMask = 5, DistanceMask = 1, GrowAndShrink = 12, HeightMask = 2, ImageMask = 0, NoiseMask = 4, PolyMask = 16, SlopeMask = 3, Smooth = 8, StrengthTransform = 6, TerrainTexture = 9, WorldBiomeMask = 11, PROConcaveConvex = 10, PROHydraulicErosion = 7, PROCurvature = 15 }
    ...
    public class ImageMask
    {
        ...
        //Poly Mask specific
        public List<PolyMask> m_allPolyMasks = new List<PolyMask>();
        public PolyMask m_selectedPolyMaskObject;
        ...
        //Curvature settings
        public float m_curvatureBlurAmount = 0.001f;
        public float m_curvatureWorldUnits = -1;
        public float m_curvatureIntensity = 0.7f;
        ...
        //Reason: Those static variables will already automatically be reset by the image mask when needed
        ...
        public RenderTexture Apply(RenderTexture inputTexture, RenderTexture outputTexture)
        {
            ...
            switch (m_operation)
            {
                ...
                case ImageMaskOperation.NoiseMask:
                    ...
                    RenderTexture noiseRT = RenderTexture.GetTemporary(inputTexture.descriptor);
                ...
                case ImageMaskOperation.PolyMask:
                    RenderTexture.active = currentRT;
                    if (m_selectedPolyMaskObject != null)
                    {
                        m_multiTerrainOperation.GetPolyMask(m_selectedPolyMaskObject);
                    }
                    filterMat.SetTexture("_ImageMaskTex", m_multiTerrainOperation.RTbakedMask);
                    Graphics.Blit(inputTexture, outputTexture, filterMat, (int)m_blendMode);
                    filterMat.SetTexture("_ImageMaskTex", null);
                ...
                case ImageMaskOperation.GrowAndShrink:
                    //translate the grow shrink distance into a 0 to 1 value, as the shader works with U/V coordinates
                    float scalarDistance = m_growAndShrinkValue / m_multiTerrainOperation.m_range;
...
#if GAIA_2023_PRO
                    filterMat.SetTexture("_InputTex", inputTexture);
                    filterMat.SetTexture("_HeightTex", m_multiTerrainOperation.RTheightmap);
                    filterMat.SetFloat("_BlurAmount", m_curvatureBlurAmount);
                    filterMat.SetFloat("_WorldUnits", m_curvatureWorldUnits);
                    filterMat.SetFloat("_Intensity", m_curvatureIntensity);
                    Graphics.Blit(inputTexture, outputTexture, filterMat, (int)m_blendMode);
#else
                    Graphics.Blit(inputTexture, outputTexture);
#endif
        ...
        private Material GetCurrentFXFilterMaterial()
        {
            ...
            switch (m_operation)
            {
                ...
                case ImageMaskOperation.CollisionMask:
                    ...
                    break;
                case ImageMaskOperation.PolyMask:
                    //We need the FilterImageMask as material for the final operation AFTER the Poly mask has been gathered from the operation.
                    //For collection & assembly of the poly mask data, see the implementation of the collision mask operation
                    //in Apply()
                    shaderName = "Hidden/Gaia/FilterImageMask";
                    break;
                case ImageMaskOperation.StrengthTransform:
                ...
                case ImageMaskOperation.GlobalSpawnerMaskStack:
                    ...
                    break;
                case ImageMaskOperation.PROCurvature:
                    shaderName = "Hidden/Gaia/FilterCurvatureMask";
                    break;
                default:
                    break;
