    ...
    public class GaiaResource
    {
            ...
            switch (resourceType)
            {
                case GaiaConstants.SpawnerResourceType.TerrainTexture:
                    ...
                    foreach (TerrainLayer proto in terrain.terrainData.terrainLayers)
                    {
                        ...
                        if (splat != null && proto != null)
                        {
                            ...
                            {
                                if (GaiaUtils.IsSameTexture(splat.m_texture, proto.diffuseTexture, false) == true)
                                {
                                    return localTerrainIdx;
                                }
                            }
                ...
                case GaiaConstants.SpawnerResourceType.TerrainDetail:
                    ...
                    foreach (DetailPrototype proto in terrain.terrainData.detailPrototypes)
                    {
                        ...
                        {
                            if (GaiaUtils.IsSameTexture(detail.m_detailTexture, proto.prototypeTexture, false))
                            {
                                return localTerrainIdx;
                            }
                            if (GaiaUtils.IsSameGameObject(detail.m_detailProtoype, proto.prototype, false))
                            {
                                return localTerrainIdx;
                            }
                ...
                case GaiaConstants.SpawnerResourceType.TerrainTree:
                    ...
                    {
                        if (GaiaUtils.IsSameGameObject(tree.m_desktopPrefab, proto.prefab, false))
                        {
                            return localTerrainIdx;
                        }
                        localTerrainIdx++;
        ...
        public List<TreePrototype> GetTreePrototypes()
        {
            ...
            foreach (ResourceProtoTree tree in m_treePrototypes)
            {
                if (tree.m_desktopPrefab != null)
                {
                    ...
                    int lodIndex = 0;
                    switch (tree.m_navMeshLOD)
                    {
                        case ResourceProtoTree.GaiaNavMeshLOD.First:
                            lodIndex = 0;
                            break;
                        case ResourceProtoTree.GaiaNavMeshLOD.Last:
                            if (tree.m_desktopPrefab != null)
                            {
                                LODGroup lodGroup = tree.m_desktopPrefab.GetComponent<LODGroup>();
                                if (lodGroup != null)
                                {
                            ...
                            break;
                        case ResourceProtoTree.GaiaNavMeshLOD.Custom:
                            lodIndex = tree.m_customNavMeshLOD;
                            break;
                    }
                    newTree.navMeshLod = lodIndex;
        ...
        public void UpdatePrototypesFromTerrain()
        {
            ...
            for (idx = 0; idx < terrain.terrainData.treePrototypes.Length; idx++)
            {
                ...
                resourceTreeProto.m_navMeshLOD = ResourceProtoTree.GaiaNavMeshLOD.Custom;
                resourceTreeProto.m_customNavMeshLOD = terrainTreeProto.navMeshLod;
        ...
        public void AddMissingPrototypesToTerrain(Terrain terrain)
        {
            ...
            foreach (ResourceProtoTexture splat in m_texturePrototypes)
            {
                ...
                {
                    if (GaiaUtils.IsSameTexture(sp.texture, splat.m_texture, false))
                    {
                        found = true;
                    }
                }
            ...
            foreach (ResourceProtoDetail detail in m_detailPrototypes)
            {
                ...
                foreach (DetailPrototype dp in terrainDetails)
                {
                    ...
                    {
                        if (GaiaUtils.IsSameTexture(dp.prototypeTexture, detail.m_detailTexture, false))
                        {
                            found = true;
                        }
                        if (GaiaUtils.IsSameGameObject(dp.prototype, detail.m_detailProtoype, false))
                        {
                            found = true;
                        }
            ...
            foreach (ResourceProtoTree tree in m_treePrototypes)
            {
                ...
                {
                    if (GaiaUtils.IsSameGameObject(tp.prefab, tree.m_desktopPrefab, false))
                    {
                        found = true;
                    }
                }
        ...
        public void AddPrototypeToTerrain(GaiaConstants.SpawnerResourceType resourceType, int resourceIdx, Terrain terrain, bool forceAddition=false)
        {
            ...
            {
                //If it is a tree, we want to update the prototype settings (NavMeshLOD, BendFactor) even if it exists
                if (resourceType == GaiaConstants.SpawnerResourceType.TerrainTree)
                {
                    int protoIdx = PrototypeIdxInTerrain(resourceType, resourceIdx, terrain);
                    if (protoIdx != -1)
                    {
                        TreePrototype[] treePrototypes = terrain.terrainData.treePrototypes;
                        ResourceProtoTree tree = m_treePrototypes[resourceIdx];
                        TreePrototype currentProto = treePrototypes[protoIdx];
                        ...
                        bool changed = false;
                        if (currentProto.navMeshLod != lodIndex)
                        {
                            currentProto.navMeshLod = lodIndex;
                            changed = true;
                        }
                        if (currentProto.bendFactor != tree.m_bendFactor)
                        {
                            currentProto.bendFactor = tree.m_bendFactor;
                            changed = true;
                        }
