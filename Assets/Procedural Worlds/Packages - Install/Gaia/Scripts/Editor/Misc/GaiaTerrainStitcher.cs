...
namespace Gaia
{
    ...
    public class GaiaTerrainStitcher : EditorWindow, IPWEditor
    {
        ...
        [HideInInspector]
        public bool autoManagerProcessing = false; // Flag to indicate automatic workflow of the tool from the Manager Window
        ...
        private void DrawTerrainStitcher(bool helpEnabled)
        {
            ...
            {
                enabledInManager = EditorGUILayout.ToggleLeft("Enable Terrain Stitcher", enabledInManager);
                if (!enabledInManager)
                {
            ...
            if (!autoManagerProcessing && m_editorUtils.Button("StartStitching"))
            {
                StartAnalyzing();
            }
        ...
        public void StartStitchTerrain(Terrain terrain, Terrain neighborTerrain)
        {
