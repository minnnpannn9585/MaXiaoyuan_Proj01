...
namespace Gaia
{
    ...
    public class SpawnerEditor : PWEditor, IPWEditor
    {
            ...
            switch (sr.m_resourceType)
            {
                case SpawnerResourceType.TerrainTexture:
                    ...
                    if (oldTexture != null)
                    {
                        ...
                        {
                            if (GaiaUtils.IsSameTexture(oldTexture, proto.diffuseTexture, false))
                            {
                ...
                case SpawnerResourceType.TerrainDetail:
                    ...
                    if (oldTexture != null || oldGameObject != null)
                    {
                        ...
                        foreach (DetailPrototype proto in activeTerrain.terrainData.detailPrototypes)
                        {
                            ...
                            {
                                if (GaiaUtils.IsSameTexture(oldTexture, proto.prototypeTexture, false) == true)
                                {
                                    detailPrototypeID = localTerrainIdx;
                                    break;
                                }
                ...
                case SpawnerResourceType.TerrainTree:
                    ...
                    if (treePrototypeID != -1)
                    {
                        foreach (Terrain t in Terrain.activeTerrains)
                        {
                            ...
                            int lodIndex = 0;
                            switch (resourceProtoTree.m_navMeshLOD)
                            {
                                ...
                                case ResourceProtoTree.GaiaNavMeshLOD.Last:
                                    ...
                                    {
                                        LODGroup lodGroup = resourceProtoTree.m_desktopPrefab.GetComponent<LODGroup>();
                                        if (lodGroup != null)
                                        {
                                ...
                                case ResourceProtoTree.GaiaNavMeshLOD.Custom:
                                    lodIndex = resourceProtoTree.m_customNavMeshLOD;
                                    break;
                            }
                            exisitingPrototypes[treePrototypeID].navMeshLod = lodIndex;
        ...
        private void DrawTreePrototype(Spawner spawner, bool showHelp)
        {
            ...
            ResourceProtoTree treeProto = m_spawner.m_treeResourcePrototypeBeingDrawn;
            EditorGUI.BeginChangeCheck();
            treeProto.m_navMeshLOD = (ResourceProtoTree.GaiaNavMeshLOD)m_editorUtils.EnumPopup("NavMeshLOD", treeProto.m_navMeshLOD, showHelp);
            if (treeProto.m_navMeshLOD == ResourceProtoTree.GaiaNavMeshLOD.Custom)
            {
                LODGroup lodGroup = treeProto.m_desktopPrefab.GetComponent<LODGroup>();
                int lodLevels = 3;
                if (lodGroup != null)
                {
                    lodLevels = lodGroup.lodCount - 1;
                }
                treeProto.m_customNavMeshLOD = m_editorUtils.IntSlider("NavMeshLODIndex", treeProto.m_customNavMeshLOD, 0, lodLevels, showHelp);
            }
            if (EditorGUI.EndChangeCheck())
            {
                EditorUtility.SetDirty(m_spawner.m_settings);
